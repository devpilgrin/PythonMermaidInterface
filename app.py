...
OUTPUT_DIR.mkdir(exist_ok=True)  # Создаем папку output при старте
...
async def generate_diagram(code, output_format):
    ...
    try:
        ...
        if not data:
            raise ValueError("Получены пустые данные от рендера")
        ...
        with gr.Column(scale=2):
            ...
            with gr.Row():
                for plugin in plugins:
                    gr.Button(
                        plugin.button_text,
                        variant="secondary",  # Исправлено: убраны нестандартные параметры
                        size="sm"
                    ).click(
